...
const DateRangeContainer: React.FC<DateRangeContainerProps> = ({
  ...
  const initialDateRangeComputed: DateRange = {
    startDate: dayjs(enteringDate).toDate(),
    endDate: dayjs(leavingDate).toDate(),
  }
    ...
    <Box sx={{ position: 'relative', minWidth: 200 }}>
      <Stack spacing={2} direction="row">
        <TextField
          ...
          value={dayjs(enteringDate).format('YYYY-MM-DD')}
          size="small"
        />
        <TextField
          label={t('To')}
          variant="outlined"
          onFocus={handleToggle}
          value={dayjs(leavingDate).format('YYYY-MM-DD')}
          size="small"
        />
      </Stack>
      {open && (
        <Box sx={{ position: 'absolute', right: 0 }}>
          <Box sx={{ position: 'relative', zIndex: 10000 }}>
            <DateRangePicker
              ...
              locale={locale}
              minDate={addMonths(parseISO(leavingDate), -2)}
              maxDate={addMonths(parseISO(enteringDate), 2)}
            />
          </Box>
        </Box>
      )}
